...
class _Collector<T, A, R> implements Collector<T, A, R> {
    ...
    constructor(supplier: JSupplier<A>, accumulator: JBiFunction<A, T, A>, finisher: JFunction<A, R>) {
...
export class Collectors {
    static toList<T>(): Collector<T, Array<T>, Array<T>> {
        return new _Collector<T, Array<T>, Array<T>>(() => [], (l, r) => {
            l.push(r);
            return l;
        }, (t) => t);
    }
...
class _ChainedSink<T, E_OUT> extends _Sink<T> {
    ...
    constructor(downstream: _Sink<E_OUT>, consumer?: JBiConsumer<T, _ChainedSink<T, E_OUT>>) {
        super();
...
class _FlatMapChainedSink<T, E_OUT> extends _ChainedSink<T, E_OUT> {
    ...
    constructor(downstream: _Sink<E_OUT>, consumerNotSized: JBiConsumer<T, _FlatMapChainedSink<T, E_OUT>>) {
        super(downstream, () => {
        });
...
class _OpFlagInfo {
    ...
    constructor(bitPosition: number, set: number, clear: number, preserve: number) {
...
abstract class _AbstractPipeline<E_IN, E_OUT> {
    static MSG_STREAM_LINKED: string = "stream has already been operated upon or closed";
    ...
    constructor(previousStage: _AbstractPipeline<any, any> | null, opFlags: number, sourceIterator?: BaseIterator<any>) {
        if (previousStage === null) {
            ...
            if (previousStage.linkedOrConsumed) throw new Error(_AbstractPipeline.MSG_STREAM_LINKED);
            ...
            this.combinedFlags = _OpFlag.combineOpFlags(opFlags, previousStage.combinedFlags);
    ...
    evaluate<R>(terminalOp: _TerminalOp<E_OUT, R>): R {
        if (this.linkedOrConsumed) throw new Error(_AbstractPipeline.MSG_STREAM_LINKED);
    ...
    copyIntoWithCancel<P_IN>(wrappedSink: _Sink<P_IN>, spliterator: BaseIterator<P_IN>) {
...
//--extension--method----
    ...
    map<R>(mapper: JFunction<P_OUT, R>): DartStream<R> {
        return new _StatelessOp<P_OUT, R>(this, _OpFlag.NOT_SORTED | _OpFlag.NOT_DISTINCT,
            (flag, sink) => {
    ...
    reduce<U>(identity: U, accumulator: JBiFunction<U, P_OUT, U>): U {
        return this.evaluate(new _ReduceOp<P_OUT, U, _TerminalSink<P_OUT, U>>(identity, accumulator));
    }

    reduce0(accumulator: JBinaryOperator<P_OUT>): P_OUT | null {
        return this.evaluate(new _ReduceOp<P_OUT, P_OUT, _TerminalSink<P_OUT, P_OUT>>(undefined, accumulator));
    }
...
class _StatelessOp<E_IN, E_OUT> extends _DsPipeline<E_IN, E_OUT> {
    ...
    constructor(previousStage: _AbstractPipeline<any, any> | null, opFlags: number, wrapSinkFunction: JBiFunction<number, _Sink<E_OUT>, _Sink<E_IN>>) {
...
class _ReduceOp<T, R, S extends _TerminalSink<T, R>> extends _TerminalOp<T, R> {
    ...
    makeSink(): _TerminalSink<T, R> {
        if (this.seed !== undefined) return new _HasSeedReducingSink<T, R>(this.seed, this.reducer);
        else return new _NoSeedReducingSink<R>(this.reducer as unknown as JBinaryOperator<R>) as unknown as _TerminalSink<T, R>;
    }
}
...
class _SliceOp<T> extends _DsPipeline<T, T> {
    ...
    constructor(previousStage: _AbstractPipeline<any, any> | null, skip: number, limit: number) {
...
class _SortedOp<T> extends _DsPipeline<T, T> {
    ...
    constructor(previousStage: _AbstractPipeline<any, any> | null, comparator: JComparator<T>, isNaturalSort: boolean) {
    ...
    opWrapSink(flags: number, sink: _Sink<T>): _Sink<T> {
        if (_OpFlag.SORTED.isKnown(flags) && this.isNaturalSort)
            return sink;
        else if (_OpFlag.SIZED.isKnown(flags))
            return new _SortedSink<T>(sink, this.comparator, true);
        else
            return new _SortedSink<T>(sink, this.comparator, false);
    }
}
...
class _SortedSink<T> extends _ChainedSink<T, T> {
    ...
    constructor(downstream: _Sink<T>, comparator: JComparator<T>, sized: boolean) {
        super(downstream);
        this.comparator = comparator;
        this.sized = sized;
    }
...
class _ReverseOp<T> extends _DsPipeline<T, T> {
    ...
    opWrapSink(flags: number, sink: _Sink<T>): _Sink<T> {
        if (_OpFlag.SIZED.isKnown(flags))
            return new _ReverseSink<T>(sink, true);
        else
            return new _ReverseSink<T>(sink, false);
    }
}
...
class _ShuffleOp<T> extends _DsPipeline<T, T> {
    ...
    opWrapSink(flags: number, sink: _Sink<T>): _Sink<T> {
        if (_OpFlag.SIZED.isKnown(flags))
            return new _ShuffleSink<T>(sink, true);
        else
            return new _ShuffleSink<T>(sink, false);
    }
}
...
class _TakeWhileOp<T> extends _DsPipeline<T, T> {
    ...
    constructor(previousStage: _AbstractPipeline<any, any>, predicate: JPredicate<T>) {
